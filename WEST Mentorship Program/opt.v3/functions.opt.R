...
find_ideal_module = function(x1, x2) {
  ...
  unique_topic = unique(v1, v2)

  combos_topic1 = combinations(length(unique_topic), length(unique_topic)-1, v=unique_topic, repeats.allowed=FALSE)
  combos_topic2 = combinations(length(unique_topic), length(unique_topic)-1, v=unique_topic, repeats.allowed=FALSE)
  ...
  colnames(df) = paste(rep(c("mod1","mod2"), each = n_topics),"sk",rep(seq(1:n_topics), times = 2),sep="")
  
  df$n_matches = NA # Ideally this number would be 2*n mentees
  n_topics  = as.numeric(length(unique_topic)-1)
  
  for(i in 1:nrow(df)){
      
      t1 = as.character(unlist(c(df[i,1:n_topics])))
      t2 = as.character(unlist(c(df[i,(n_topics+1):(n_topics*2)])))
...
define_modules_v2 = function(mod_topic_df, topics1, topics2) {
  ...
  # Will this number of rooms accommodate all mentees? 
...
define_modules_v3 = function(topics1, topics2, min_mentors) {
  ...
  rooms_mod1 = data.frame(topic = as.character(unique(topics1)),
                          rooms = NA)
  rooms_mod1$topic = as.character(rooms_mod1$topic)
  rooms_mod1 = rooms_mod1 %>%
    filter(!is.na(topic)) %>% arrange(topic)
  ...
  rooms_mod2 = data.frame(topic = as.character(unique(topics2)),
                          rooms = NA)
  rooms_mod2$topic = as.character(rooms_mod2$topic)
  rooms_mod2 = rooms_mod2 %>%
    filter(!is.na(topic)) %>% arrange(topic)
  ...
  req_rooms_mod1 = sum(rooms_mod1$rooms)
  req_rooms_mod2 = sum(rooms_mod2$rooms)
  
  ## If the required number of rooms is less than the variable min_mentors,
  # stop. Or else continue with optimization.
  
  if(req_rooms_mod1 < min_mentors & req_rooms_mod2 < min_mentors){
    
    # Prep the room dataframes for joining
    
    r1 = c()
    for(i in 1:nrow(rooms_mod1)){
      r = rep(rooms_mod1$topic[i], each = rooms_mod1$rooms[i])
      r1 = c(r1, r)
    }
    
    rooms1 = data.frame(topic = r1,
                        breakout_room = seq(1,length(r1), by =1),
                        mod = "Module1")
    r2 = c()
    for(i in 1:nrow(rooms_mod2)){
      r = rep(rooms_mod2$topic[i], each = rooms_mod2$rooms[i])
      r2 = c(r2, r)
    }
    
    rooms2 = data.frame(topic = r2,
                        breakout_room = seq(1,length(r2), by = 1),
                        mod = "Module2")
    
    
    
    long_final_df = rbind(rooms1, rooms2)
    long_final_df$mod = as.character(long_final_df$mod)
    long_final_df$topic = as.character(long_final_df$topic)
...
select_mentors_v2 = function(mentor_pref_df, modules_defined) {
...
assign_subskills  = function(mentor_mod_df, mentee_pref_df1, mentee_pref_df2) {
  ...
  mentee_pred_df1$Ranking = as.integer(mentee_pred_df1$Ranking)
  ...
  while(count < length(df$mod[df$mod=="Module1"])){
   ...
   slice = mentee_pred_df1 %>%
     filter(Skill==skills$topic[i]) %>%
     arrange(Ranking, desc(n_ranking)) %>%
     select(Skill, Skill.SubSkill)
  
   
   df[count:((count+skills$Freq[i])-1),c(5)] = slice[1:skills$Freq[i],c(2)]
...
assign_mentees_v2 = function(mentor_mod_df, mentee_df, module) {
  ...
  # Next we want to subset to only the skills + subskills included in module 1
  m1 = mentee_df %>%
    filter((!!Skill.Set.Other)!=Skill & !is.na(Ranking)) %>% # For whatever reason, we need to specify this as a reverse argument *shrug*.
    #filter(Skill.SubSkill== mentor_mod_df$Skill.SubSkill) %>%
    #select(-(!!Skill.Set)) %>%
    group_by(Mentee.Name) %>%
    add_tally(Skill.SubSkill %in% mentor_mod_df$Skill.SubSkill[mentor_mod_df$mod==module], name = "n_skills") %>%
    #mutate(n_skills = n(Skill.SubSkill %in% mentor_mod_df$Skill.SubSkill[mentor_mod_df$mod==module])) %>%
    #mutate(n_skills = n()) %>%
...
while(count1 < 3) {  
if(length(unassigned_mentees) > 0) {  # Do this until we assign all possible mentees.
  
  # Remove mentees that did not match on mod1 subskills from first cut consideration (they'll get prioritized in the next round)
  unassigned_mentees = unique_mentee[!(unique_mentee %in% no_assignment$Mentee.Name)] 
  
  # Get a random shuffling of rows.
  m1d_long = m1d_long_orig %>% 
    filter(mod == module) %>%
    sample_frac(1)
...
for(i in 1:3){    # Assign mentees in such a way that prioritizes first choices
  ...
  while(count < length(m1d_long$mod[m1d_long$mod==module & is.na(m1d_long$Mentee.Name)])) {
    ...
    count = count + 1
    slice = m1 %>%
      filter(Mentee.Name %in% unassigned_mentees) %>%
      filter(Skill.SubSkill == m1d_long$Skill.SubSkill[is.na(m1d_long$Mentee.Name)][count]) %>%
      filter(Ranking==i) %>%
      arrange(desc(prob)) %>%
      slice(n=1) %>%
  ...
  while(try < 6) {  # Try five times.
    ...
    # Remove mentees that did not match on mod2 subskills  from first cut consideration (they'll get prioritized next)
